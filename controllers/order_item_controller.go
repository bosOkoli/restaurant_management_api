...
func GetOrderItemsByOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listing error items by order"})
			return
		}
		c.JSON(http.StatusOK, allOrderItems)
	}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	unwindTableStage := bson.D{{"$unwind", bson.D{{"path", "$table"}, {"preserveNulAndEmptyArrays", true}}}}
	...
	groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"order_d", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}},
